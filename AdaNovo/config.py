...
class Config:
    """The Adanovo configuration options."""
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        for key, val in self._config_types.items():
            if key == "ptm_types":
                ptm_set = set(self._user_config['ptm_types'])
                residues_set = set(self._user_config['residues'])
                if len(ptm_set) == 0:
                    logger.info("No specific ptm types!")
                else:
                    difference = ptm_set - residues_set
                    if difference: 
                        logger.error(
                            "PTM types that are not exist in config.residues!", list(difference)
                        )
                        raise ValueError(
                            f"Incorrect ptm types: {list(difference)}"
                        )
            else:
