...
from depthcharge.components.transformers import generate_tgt_mask

class CausalTransformerEncoderLayer(torch.nn.TransformerEncoderLayer):
    def __init__(self, *args, **kwargs):
        super(CausalTransformerEncoderLayer, self).__init__(*args, **kwargs)
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None,is_causal=False):
        ...
        return super(CausalTransformerEncoderLayer, self).forward(src, src_mask, src_key_padding_mask)


class DecoderOnlyPeptide(_PeptideTransformer):
    def __init__(
        self,
        dim_model=512,
        n_head=8,
        ...
        layer = CausalTransformerEncoderLayer(
            d_model=dim_model,
            nhead=n_head,
    ...
    def forward(self, sequences):
        ...
        preds = self.transformer_encoder(tgt)
...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    **kwargs : Dict
    ...
    def __init__(
        ...
        s1: str = "0.3",
        s2: str = "0.1",
        ptm_types: List[str] = ["M(+15.99)", "N(+.98)", "Q(+.98)"],
        **kwargs: Dict,
    ):
        super().__init__()
        ...
        ########## 4124
        self.decoder_only = DecoderOnlyPeptide(
            dim_model=dim_model,
            n_head=n_head,
            dim_feedforward=dim_feedforward,
            n_layers=n_layers,
    ...
    def _cache_finished_beams(
        ...
        for i in range(len(beams_to_cache)):
            ...
            ########## 7352
            cache_score = [elem[0] for elem in pred_cache[spec_idx]]
            if peptide_score in cache_score:
                pass
            else:
            ########## 7352
    ...
    def _forward_step(
        ...
        """
        ########## 9755
        return self.decoder(sequences, precursors, *self.encoder(spectra)), self.decoder_only(sequences)
        # return self.decoder(sequences, precursors, *self.encoder(spectra))
        ########## 9755


    def new_loss(self,pred_t,truth,pred_d):
        ...
        ### 计算loss_1
        # 提取真实标签的概率
        b_t, l_t, v_t = pred_t.size()
        b_idx = torch.arange(b_t).unsqueeze(1).expand(b_t, l_t)
        l_idx = torch.arange(l_t).unsqueeze(0).expand(b_t, l_t)
        
        truth_mask = torch.where(truth != 0, torch.tensor(1.0), truth)
        non_zero_row = torch.sum(truth_mask, dim=1, keepdim=True)
        nonzero_count = torch.sum(non_zero_row)

        predt_probs = F.softmax(pred_t, dim=-1)
        truth_probs_t = predt_probs[b_idx, l_idx, truth]  # (B,L)
        # truth_probs_t_1 = -torch.log(truth_probs_t) * truth_mask
        
        predd_probs = F.softmax(pred_d, dim=-1)
        truth_probs_d = predd_probs[b_idx, l_idx, truth] # (B,L)

        # 计算互信息大小
        I =  torch.log(truth_probs_t / truth_probs_d) # (B,L)
        mask_I = I * truth_mask
        mean_I = torch.sum(mask_I, dim=1, keepdim=True) / non_zero_row
        std_I = torch.sqrt(torch.sum( torch.square( (mask_I - mean_I) * truth_mask ), dim=1, keepdim=True ) \
                                    / (non_zero_row - 1) ).to(mask_I.device)


        # 计算W_aa(aa_level)
        # import pdb;pdb.set_trace()
        normalized_I = (mask_I - mean_I) / std_I
        W_aa = (normalized_I * self.s1 + 1) * truth_mask # (B, L), mean = 1, std = s1
        W_aa[W_aa < 0] = 0 

        # 计算W_psm(psm_level)
        # I_psm = I.mean(dim=1,keepdim=True)
        W_psm_mean = mean_I.mean()
        W_psm_std = mean_I.std()
        W_psm = ( (mean_I - W_psm_mean) / W_psm_std )* self.s2 + 1 # (B, 1), mean = 1, std = s2
        W_psm[W_psm < 0] = 0 # (B, 1)

        W_j = W_aa * W_psm #(B, L)
       
        loss_1 =  ( - W_j * torch.log(truth_probs_t) ).sum() / nonzero_count

        pred_d = pred_d.reshape(-1, self.decoder.vocab_size + 1)
        loss_2 = self.celoss(pred_d, truth.flatten())    
    ...
    def training_step(
        ...
        *args,
        ...
        """
        (pred_t, truth), pred_d = self._forward_step(*batch)
        assert (len(pred_t) == len(truth))
        assert (len(pred_t) == len(pred_d))
        
        if mode == 'train':
            loss = self.new_loss(pred_t, truth, pred_d)
        elif mode == 'valid':
            pred_t = pred_t[:, :-1, :].reshape(-1, self.decoder.vocab_size + 1)
            loss = self.celoss(pred_t, truth.flatten())
    ...
    def validation_step(
        ...
        aa_precision, aa_recall, pep_precision, ptm_recall, ptm_precision = evaluate.aa_match_metrics(
            *evaluate.aa_match_batch(
                peptides_true, peptides_pred, self.decoder._peptide_mass.masses, self.ptm_types
            )
        )
        ...
        self.log(
            "AA recall at coverage=1", {"valid": aa_recall}, **log_args
        )
        self.log(
            "PTM recall at coverage=1", {"valid": ptm_recall}, **log_args
        )
        self.log(
            "PTM precision at coverage=1", {"valid": ptm_precision}, **log_args
        )
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        metrics = {
            ...
            "valid_aa_recall": callback_metrics[
                "AA recall at coverage=1"
            ]["valid"].detach(),
            "valid_pep_precision": callback_metrics[
                "Peptide precision at coverage=1"
            ]["valid"].detach(),
            "valid_ptm_recall": callback_metrics[
                "PTM recall at coverage=1"
            ]["valid"].detach(),
            "valid_ptm_precision": callback_metrics[
    ...
    def _log_history(self) -> None:
        ...
        if len(self._history) == 1:
            logger.info(
                "Step\tTrain loss\tValid loss\tPeptide precision\tAA precision\tAA recall\tPTM recall\tPTM precision"
            )
        metrics = self._history[-1]
        if metrics["step"] % self.n_log == 0:
            logger.info(
                "%i\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f",
                metrics["step"],
            ...
            if self.tb_summarywriter is not None:
                for descr, key in [
                    ...
                    ("eval/val_aa_recall", "valid_aa_recall"),
                    ("eval/val_ptm_recall", "valid_ptm_recall"),
                    ("eval/val_ptm_precision", "valid_ptm_precision"),
                ]:
